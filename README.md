...
网页最上面的“定投策略”可以在两个策略之间切换。两个策略都用**定投日前一个交易日的收盘价**计算，不会偷看当天价格；默认每周一定投，基础金额 $100。
...
- 提醒按 `docs/strategy.js` 里的 `DEFAULT_CONFIG` 计算，`strategy` 决定主提醒按哪个策略（`"tiered"` 跌多多投，`"trend"` 趋势定投）。你在网页上改的设置只存在自己的浏览器里，GitHub 看不到；想让提醒也按新设置算，就改 `DEFAULT_CONFIG`。
...
**想改默认规则（网页第一次打开时的设置）？**
改 `docs/strategy.js` 最上面的 `DEFAULT_CONFIG`。比如想默认用跌多多投，把 `strategy: "trend"` 改成 `strategy: "tiered"`。在 GitHub 网页上点开这个文件，点铅笔图标编辑，然后 **Commit changes**。提交后网站会自动重新发布，定投日提醒也会跟着变。
...
**想检查计算逻辑？**
```bash
node --test tests/strategy.test.js            # 两个策略、回测和买入记录（23 项）
node --test tests/reminder.test.js            # 定投日提醒（7 项）
